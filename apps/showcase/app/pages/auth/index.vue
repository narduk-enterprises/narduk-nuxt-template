...
async function onDemoLogin() {
  ...
  try {
    ...
    await fetchSession()
    await navigateTo('/auth/dashboard/', { replace: true })
  } catch (err: unknown) {
...
<template>
  <div class="flex flex-col items-center justify-center min-h-[60vh] space-y-8">
    ...
    <div class="flex items-center gap-4">
      <UButton to="/auth/login" size="lg" icon="i-lucide-log-in"> Sign In </UButton>
      <UButton
        to="/auth/register"
        size="lg"
        variant="outline"
    ...
    <NuxtLink to="/" class="text-sm text-muted hover:text-primary transition-colors">
      ← Back to Showcase
    </NuxtLink>
  </div>
</template>
